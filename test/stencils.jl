...
function test_funcs(name, f, args...; broken = false)
    ...
    if broken
        @test_broken open(x -> read(x, String), "build_function_tests/$name-outplace.jl") ==
                     _repr(outplace)
        @test_broken open(x -> read(x, String), "build_function_tests/$name-inplace.jl") ==
                     _repr(outplace)
    else
...
@testset "array codegen basics" begin
    ...
    @makearray y[1:6, 1:6] begin y[2:(end - 1), 2:(end - 1)] => @arrayop (i, j) x[j, i] end
    ...
    @makearray y[1:5, 1:5] begin
        y[:, :] => 0
        y[2:(end - 1), 2:(end - 1)] => @arrayop (i, j) (x[i + 1, j] + x[i - 1, j] +
                                                        x[i, j + 1] + x[i, j - 1])/2
    end
