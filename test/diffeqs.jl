using Symbolics
using Symbolics: solve_linear_ode_system, SymbolicLinearODE, has_const_coeffs, to_homogeneous, symbolic_solve_ode, find_particular_solution, IVP, solve_symbolic_IVP
...
evalsol(x) = SymbolicUtils.evaluate(x)

# Systems
# ideally, `isapprox` would all be `isequal`, but there seem to be some floating point inaccuracies
@test all(isapprox.(evalsol.(solve_linear_ode_system([1 0; 0 -1], [1, -1], t)), [exp(t), -exp(-t)]))
@test all(isapprox.(evalsol.(solve_linear_ode_system([-3 4; -2 3], [7, 2], t)), [10exp(-t) - 3exp(t), 5exp(-t) - 3exp(t)]))
@test all(isapprox.(evalsol.(solve_linear_ode_system([4 -3; 8 -6], [7, 2], t)), [18 - 11exp(-2t), 24 - 22exp(-2t)]))
...
@test all(isapprox.(expand.(solve_linear_ode_system([1 -1 0; 1 2 1; -2 1 -1], [7, 2, 3], t)), (5//3)*exp(-t)*[-1, -2, 7] - 14exp(t)*[-1, 0, 1] + (16//3)*exp(2t)*[-1, 1, 1]))
...
@test isequal(evalsol.(expand.(solve_linear_ode_system([1 -1 0; 1 2 1; -2 1 -1], [7, 2, 3], t))), (5//3)*exp(-t)*[-1, -2, 7] - 14exp(t)*[-1, 0, 1] + (16//3)*exp(2t)*[-1, 1, 1])
...
# LinearODEs
@test Symbolics.is_homogeneous(SymbolicLinearODE(x, t, [1, 1], 0))
@test !Symbolics.is_homogeneous(SymbolicLinearODE(x, t, [t, 1], t^2))

@test has_const_coeffs(SymbolicLinearODE(x, t, [1, 1], 0))
@test !has_const_coeffs(SymbolicLinearODE(x, t, [t^2, 1], 0))

@test Symbolics.is_homogeneous(to_homogeneous(SymbolicLinearODE(x, t, [t, 1], t^2)))
...
@test isequal(evalsol.(symbolic_solve_ode((Dt^2)(x) + 2(Dt^1)(x) + x ~ 0, x, t)), C[1]*exp(-t) + C[2]*t*exp(-t))
@test isequal(evalsol.(symbolic_solve_ode(SymbolicLinearODE(x, t, [0, 0, 0, 4, -4], 0))), C[1] + C[2]*t + C[3]*t^2 + C[4]*exp(2t) + C[5]*t*exp(2t))
@test isequal(evalsol.(symbolic_solve_ode(SymbolicLinearODE(x, t, [8, 12, 6], 0))), C[1]*exp(-2t) + C[2]*t*exp(-2t) + C[3]*t^2*exp(-2t))

## complex characteristic roots
@test isequal(evalsol(symbolic_solve_ode(SymbolicLinearODE(x, t, [1, 0], 0))), C[1]*cos(t) + C[2]*sin(t))
@test isequal(evalsol(symbolic_solve_ode(SymbolicLinearODE(x, t, [0, 1, 0], 0))), C[1] + C[2]*cos(t) + C[3]*sin(t))
...
# Parsing
@test isequal(SymbolicLinearODE(x, t, [1], 0), SymbolicLinearODE(Dt(x) + x ~ 0, x, t))
...
# IVP
@test isequal(solve_symbolic_IVP(IVP(SymbolicLinearODE(x, t, [-3, 2], 0), [1, -1])), (1//2)exp(-3t) + (1//2)exp(t))
@test isequal(solve_symbolic_IVP(IVP(SymbolicLinearODE(x, t, [9, -6], 4exp(3t)), [5, 6])), 5exp(3t) - 9t*exp(3t) + 2(t^2)*exp(3t))
...
@test Symbolics.is_solution(C[1]*exp(3t) + C[2]*t*exp(3t) + 2(t^2)*exp(3t), SymbolicLinearODE(x, t, [9, -6], 4exp(3t)))
