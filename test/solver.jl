...
@testset "solving tests" begin
    ...
    function correctAns(p, a)
        isequal(sort(Symbolics.convert_solutions_to_floats(p)), a) && !hasFloat(p)
    end
    ...
    @test correctAns(solve_single_eq(2 * x^2 - 6 * x - 7 ~ 0, x),
                     [(3.0 / 2.0) - sqrt(23.0) / 2.0, (3.0 / 2.0) + sqrt(23.0) / 2.0])
    #functions inverses
    ...
    #strange
    @test correctAns(solve_single_eq(sin(x + 2 // 5) + cos(x + 2 // 5) ~ 1 // 2, x),
                     [acos(0.5 / sqrt(2.0)) + 3.141592653589793 / 4.0 - (2.0 / 5.0)])
    #product
