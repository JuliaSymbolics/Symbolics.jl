...
@test isequal(dstar_derivative(sin(cos(x))*cos(cos(x)), x), expand_derivatives(D(sin(cos(x))*cos(cos(x)))))
...
@test_broken isequal(dstar_derivative(2sin(x^4 - x) + 3cos(2x), x), expand_derivatives(D(2sin(x^4 - x) + 3cos(2x))))
@test_broken isequal(dstar_derivative(2x*exp(x) + 2*exp(x), x), expand_derivatives(D(2x*exp(x) + 2*exp(x))))