...
function derivative(::typeof(abs), args::NTuple{1, Any}, ::Val{1})
    IfElse.ifelse(signbit(args[1]), -one(args[1]), one(args[1]))
end
...
function derivative(::Union{typeof(ceil), typeof(floor), typeof(factorial)},
                    args::NTuple{1, Any}, ::Val{1})
    zero(args[1])
end
